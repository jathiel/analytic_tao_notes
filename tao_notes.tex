...
\end{align}
Therefore, by~\eqref{final_form_limit}, to get the desired result, it is enough to show that $(s-1)K(s,N)\to 1$ as $N\to\infty$. A simple calculation shows that
\begin{align*}
(s-1)K(s,N) &=\int_1^N\frac{(s-1)^2\log{x}}{x^s}\;dx\\
&= -\frac{(s-1)\log{x}+1}{x^{s-1}}\bigg|_1^N\\
&= 1-\frac{(s-1)\log{N}+1}{N^{s-1}},
\end{align*}
which gives the desired limit. This shows that the inequality in~\eqref{final_form_limit} can be made arbitrarily small as $s\to1^+$.
\end{proof}
